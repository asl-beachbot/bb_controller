...
class controller
{
  ...
  const static double absolute_velocity_ = 0.17;
...
//            }
	...
	ROS_INFO("stopping_act %d", stopping_activator);
    ...
    for(;k<2;k++){
        ...
        if(start_toggle_ == true){
            ...
            search_range_ahead_applied_ = (int)(path_with_information_help.size() - indices_now_help) / 7;
            start_toggle_ = false;
        }
        else{
...
//    }
	...
	ROS_INFO("anglediff: %f, %f", angledifference, absolute[0]);
...
//    Velocityoutput.angular.z = ang[0];
    ...
    if(k >= 1){
      ...
      if(temp_unsigned == 128 || temp_unsigned == 129){
          temp_unsigned = 0;
      }
