...
class controller
{
  vector<geometry_msgs::Twist> path_with_information; //linear.x&y are pathpoints coordinates, linear.z is rake information
  ...
  // Controller Gains:
    ...
    const static int search_range_ahead_ = 30;
  ...
  void pose_sub_Callback(const geometry_msgs::PoseStamped &laserpose)
  {
    ...
    for(;k<2;k++){
        ...
        if(start_toggle_ == true){
            ...
            search_range_ahead_applied_ = (int)(path_with_information_help.size() - indices_now_help) / 8;
            start_toggle_ = false;
        }
        else{
    ...
    if(indices_now_platform > path_with_information.size() - 5){
        ang[0] = 0;
        absolute[0]=0;
    }
...
//    }
    ...
    if(k == 1){
    Velocityoutput.rake = path_with_information[i_nearest].linear.z;
    }
...
//    }
  ...
  controller()
  {
    ...
    this->ReadFile("/home/beachbot/Paths/PathToStart.path", 0);
    this->ReadFile("/home/beachbot/Paths/PathToStart.path", 1);
    start_toggle_ = true;
